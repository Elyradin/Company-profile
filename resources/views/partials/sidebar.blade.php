    ...
    <ul class="nav flex-column gap-2">
        <li>
            <a class="nav-link" id="link-dashboard" href="#">
                ...
                <span class="ms-2">Profile / Dashboard</span>
            </a>
        </li>
        ...
        <li id="menu-services-admin" style="display: none;">
            <a class="nav-link" href="{{ route('admin.services') }}">
                ...
                <span class="ms-2">Services (Admin)</span>
            </a>
        </li>

        <li id="menu-services-user" style="display: none;">
        ...
        <li class="nav-item">
            <button id="logoutBtn" type="button" class="nav-link btn btn-link text-white p-0 text-start">
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!userStr || !token) {
        // Kalau tidak ada data, tendang ke login
    ...
    // 3. Atur Link Dashboard sesuai Role (Opsional, jika dashboard admin beda url)
    ...
    // 4. LOGIKA MENU BERDASARKAN ROLE
    if (user.role === 'admin') {
    ...
    document.getElementById('logoutBtn').addEventListener('click', async () => {
        ...
        try {
            // Panggil API Logout agar token di server dihapus (opsional tapi bagus)
        ...
        // Hapus Data di Browser & Redirect
        localStorage.clear();
        window.location.href = '/login';
    });
